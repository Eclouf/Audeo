...
class EmbedThumbnailPP(FFmpegPostProcessor):
    ...
    def __init__(self, downloader=None, already_have_thumbnail=False, square=True):
        super().__init__(downloader)
        self._already_have_thumbnail = already_have_thumbnail
        self._square = square

    def _make_square_thumbnail(self, thumbnail_filename):
        """Convertit la miniature en image carrée"""
        try:
            with Image.open(thumbnail_filename) as img:
                ...
                square_filename = f"{os.path.splitext(thumbnail_filename)[0]}.png"
                square_img.save(square_filename, "png", quality=95)
    
            return square_filename
                
        except Exception as e:
            self.report_warning(f'Erreur lors de la création de la miniature carrée: {e}')
    ...
    def run(self, info):
        ...
        if self._square:
            print('[ThumbnailSquare] Making square thumbnail')
            thumbnail_filename = self._make_square_thumbnail(thumbnail_filename)
            thumbnail_ext = 'png'
